...
vTournaments <- c(
    ...
    "2021_European_Championship",
    "2016_European_Championship",
    "2012_European_Championship",
    "2008_European_Championship",
    "2004_European_Championship",
...
system("./phantomjs.exe ./code/scrape_elos_2006_World_Cup.js")

# then prepare the rest of the js files
for (i in vTournaments) {
  year=i
  js_script_old_name = paste0('./code/scrape_elos_', "2006_World_Cup", '.js') # get the name of the the js file from the previous year
  js_script_new_name = paste0('./code/scrape_elos_', i, '.js') # make the name of the js file for the new year
  js_script_old_text <- readChar(js_script_old_name, file.info(js_script_old_name)$size) # read the old js file
  js_script_new_text <- gsub("2006_World_Cup",i, js_script_old_text) # make changes in the text
...
vTournaments <- append(vTournaments, "2006_World_Cup")

# then go throught the js files to download the webpages
for (i in vTournaments) {
  year=i
  system_name <- paste0('./phantomjs.exe ./code/scrape_elos_',i,'.js')
  system(system_name)
  }
...
for (i in vTournaments){
  year=i
  file_name = paste0('./web_pages_elos/',i,'.html')
...
write_csv(data, "./data/nations_elo_start_tournament.csv")
write_rds(data, "./data/nations_elo_start_tournament.rds")
